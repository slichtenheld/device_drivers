...
static ssize_t my_read(struct file *file, char __user *buf, size_t lbuf, loff_t *ppos) {
	...
	if(down_interruptible(&my_custom_struct.sem)!=0){
		int nbytes = 0;
...
static loff_t my_lseek(struct file *file, loff_t offset, int orig) {
	if(down_interruptible(&my_custom_struct.sem)!=0){
		...
		switch(orig) {
			...
			case SEEK_END: testpos = ramdisk_size + offset; break; // EOF
			default: return -EINVAL;
		}
		if (testpos >= ramdisk_size) { // needs to realloc memory 
			while (testpos >= ramdisk_size) { // increase size until pointer is within bounds
				ramdisk_size += 4*PAGE_SIZE;
			}
			krealloc( my_custom_struct.ramdisk, ramdisk_size, GFP_KERNEL );
...
static int __init driver_entry(void) {
	...
	my_custom_struct.ramdisk = kzalloc(ramdisk_size, GFP_KERNEL);
